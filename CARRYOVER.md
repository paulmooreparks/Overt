...
What exists today, pinned by 346 passing tests:
...
- **Cross-file modules via `use module.{sym1, sym2}`.** Per-file compilation now walks a module graph (ModuleGraph), resolves sibling `.ov` files by filename, and threads each module's exported symbols through to the next one's name resolver and type checker. The emitter adds `using static Overt.Generated.<Module>.Module;` for each import so the generated C# compiles as one coherent assembly. `overt run main.ov` handles this end-to-end; other emit modes are single-file. Wildcard imports are forbidden (DESIGN.md §19); dotted paths and aliasing are future work.
...
1. **Dotted module paths + aliasing** (1 session). `use stdlib.http.client` (directory walk) and `use stdlib.http as http`. Additive grammar, search-path walk, and an alias-aware resolver. Unlocks a blessed-stdlib layout where facades live under `stdlib/`.
2. **Single-file emit paths pick up imports** (½ session). Today `--emit=csharp foo.ov` on a file with `use` decls silently drops them. Wire ModuleGraph through the emit path so it works, or diagnose clearly.
