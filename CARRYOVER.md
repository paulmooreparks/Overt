...
3. **C# emitter: IIFE-wrapping context loss** (1-2 sessions). A cluster of related quirks where the emitter wraps a match/if body in `((Func<...>)(() => { ... }))()` and loses surrounding context. All have one-line source-level workarounds in `samples/diffconf` and `samples/envcheck`; a focused emitter pass would clear them as a group. The instances surfaced so far:
   - Side-effect-only `if` inside a for-each loop body emits as a ternary expression-statement, hitting C# CS0201. Workaround: lift the if into expression position by assigning the result back (`x = if cond { f(x) } else { x }`).
   - Side-effect-only `match` inside a for-each loop body — same root issue, same workaround.
   - `?`-propagation inside a for-each inside an IIFE-wrapped match arm body degrades to `.Unwrap()` (which throws on Err) instead of a proper early-return. Workaround: each match arm body is a single fn call so no IIFE wrapping happens.
   - `Result` patterns (`Ok` / `Err`) inside an IIFE-wrapped match arm body emit without their `ResultOk<T,E>` / `ResultErr<T,E>` rewrite, breaking C# pattern syntax. Workaround: bind the scrutinee to a typed `let` before matching.
   - Generic record literal (`Pair { left = ..., right = ... }`) in some IIFE contexts emits `new Pair(...)` without the type-arg list. Workaround: define a non-generic record for the local shape.

   Common fix shape: when the emitter chooses to wrap a body in an IIFE, propagate the surrounding type/pattern context into the lambda body. Where a body is purely statement-position with no value consumed, emit a real if/match statement instead of an IIFE-wrapped expression. Each instance is its own emitter site but all touch the same lowering decision.
