...
4. **Generic-method opaque support** (1 session). `HttpClient.GetFromJsonAsync<T>(Uri)` and similar BCL methods that take type parameters are skipped by BindGenerator today. Adding a convention for declaring generic externs (either explicit type args in the binds target, or a separate facade per specialization) unlocks typed JSON interop.
   ...
   **Void-Task externs** (1 session). C# methods that return non-generic `Task` (e.g., `TcpClient.ConnectAsync(host, port)`) can't be cleanly bound from Overt today — Overt's `Task<()>` literally means `Task<Unit>`, which C# doesn't auto-convert from `Task`. Surfaced while attempting `samples/portcheck`. Fix shape: when an extern declares `-> Task<()>` and the binds-target's actual return type is non-generic Task, the emitter wraps with `await ...; return Unit.Value;` instead of trying to assign Task to Task<Unit>. Until this lands, samples that need TCP / HTTP fire-and-await go through `Process.run` + an external CLI tool instead (which is what `samples/pingall` does, awkward but works).
