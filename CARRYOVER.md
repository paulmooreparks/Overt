...
What exists today, pinned by 355 passing tests:
...
1. **Cross-type opaque extern references** (1 session). Today `overt bind` on a reference type only emits methods whose param/return types are primitives OR the target type itself. `StreamReader.Read(char[])` is skipped because `char[]` is foreign; `HttpClient.GetAsync(Uri)` is skipped because `Uri` is another opaque type. Extend BindGenerator to emit references to *other* opaque extern types — requires either a user-supplied list ("I also have extern-type facades for these types") or a workspace-level registry. Unlocks realistic HttpClient / Stream / Task-of-T usage.
2. **Value-type (struct) extern support** (1 session). `DateTime`, `Guid`, `TimeSpan` are value types — not class. Today BindGenerator only treats reference types as opaque, so instance members on `DateTime` don't emit. Option: detect value types and emit them as opaque too (they can still be passed by value through the C# boundary); or build a small hardcoded set with custom emission rules.
3. **Int64 type** (½ session). Overt's `Int` lowers to C# `int` (32-bit); BCL methods returning `long` are currently `// skipped`. Adding `Int64` unlocks `Environment.TickCount64`, `File.GetSize`, `DateTime.Ticks`, etc.
