...
What exists today, pinned by 337 passing tests:
...
**Semantic-enforcement arc is operationally complete: 13 diagnostic codes now reject real bugs across type correctness, exhaustiveness, effect rows, ignored Results, refinement predicates, and loop-control-flow placement. Real stdlib runtime is in. `?` / `|>?` lower to faithful early-return propagation, including inside if/match arms used as expression values (stmt-level restructuring). Control flow (for each / loop / break / continue) and literal patterns in match are all wired end-to-end. `AGENTS.md` is in the repo root, and every OV-code diagnostic auto-points at the relevant section. `overt fmt` emits canonical form for every example (13 idempotence tests). 337/337 tests. Godbolt release engineering staged and waiting on a tag push. The architectural tiers still outstanding: Go backend, module system, LSP, MSBuild integration.**
