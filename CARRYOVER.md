...
3. ~~**C# emitter: IIFE-wrapping context loss**~~ **Largely resolved.** Two of the five quirks shipped fixes:
   ...
   Remaining (still need a session):
   - `?`-propagation inside a for-each inside an IIFE-wrapped match arm body degrades to `.Unwrap()` (which throws on Err) instead of a proper early-return. Workaround in `samples/diffconf`: each match arm body is a single fn call so no IIFE wrapping happens.
   - Generic record literal (`Pair { left = ..., right = ... }`) in some IIFE contexts emits `new Pair(...)` without the type-arg list. Workaround in `samples/envcheck`: define a non-generic record for the local shape (which is independently good for readability anyway, so the workaround stuck).
