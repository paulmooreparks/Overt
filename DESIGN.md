...
The host ecosystem is reached **per back end**, not through a platform-neutral abstraction. This was originally implemented as curated `stdlib/<backend>/*` subtrees of `.ov` facades; that approach was retired in favor of `extern "csharp" use "..."` bulk-import (§17), which reflects on host metadata at compile time rather than committing the surface to source. The principle is unchanged; the mechanism is sharper.

- **Primary story.** Each back end exposes its native ecosystem through its own bulk-import resolver. `extern "csharp" use "System.IO.Path"` binds to .NET's `System.IO.Path`; a future `extern "go" use "os"` would bind to Go's `os` package; `extern "ts" use "fs"` to Node's `fs`. A program that writes `extern "csharp" use ...` is explicitly a .NET program — not one that happens to run on .NET today and might port tomorrow.
- **Naming inside each ecosystem mirrors the host.** .NET targets are spelled with their full `System.*` paths; Go targets will be spelled with Go package names; TS targets with Node module names. Agents writing C# Overt see C# idioms; agents writing Go Overt see Go idioms. No translation layer and no synthetic neutral names.
- **No portable stdlib in the language core.** Not as a fallback, not as a convenience. If an Overt program needs to run on two back ends, the answer is **agent-driven retargeting**: ask the agent to rewrite the program against the other back end's bulk-import surface. Cheap for agents in ways it is not cheap for humans; preserves our ability to not design a leaky abstraction.
