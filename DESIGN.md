...
- **Generated output lives under `obj/` or `generated/`, not `src/`.** A project convention, not an emitter enforcement. Build scripts should wipe/regenerate the output directory so local edits are transient. Putting transpiled files in `src/` is the single biggest enabler of the directed-edit failure mode; keeping them in a throwaway directory removes the opportunity.
