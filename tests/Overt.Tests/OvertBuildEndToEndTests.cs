...
public class OvertBuildEndToEndTests
{
    ...
    // The previous `ConfigValidate_AllFixturesProduceExpectedOutputAndExitCode`
    // test exercised the original `samples/config-validate/` (a hybrid
    // C#/Overt sample) end-to-end through `dotnet build` + `dotnet run`.
    // It served as the Phase-1 worked example for the Overt.Build
    // integration. The sample has been refreshed as `samples/valconf/`,
    // a pure-Overt CLI that runs via `overt run` rather than the
    // MSBuild path; its end-to-end coverage lives via the validator
    // logic exercised by `StdlibTranspiledEndToEndTests` and the
    // `samples/msbuild-smoke/` test above continues to cover the
    // MSBuild-integration story specifically.
