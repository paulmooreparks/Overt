...
`Set.values` is the iteration escape hatch — every other Set op
produces a Set, and without `values` you can't get the elements
back into a List for further processing. Iteration order is
hash-defined and not stable across hosts (matches `Map.keys` /
`Map.values`); programs that need a deterministic order sort the
returned list.
