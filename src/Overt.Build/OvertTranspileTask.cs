...
public sealed class OvertTranspileTask : Microsoft.Build.Utilities.Task
{
    ...
    private static string? TryBuildEntryPoint(ModuleDecl module, string sourcePath)
    {
        var mainFn = module.Declarations
            .OfType<FunctionDecl>()
            .FirstOrDefault(f => f.Name == "main" && ClassifyMainSignature(f) is not null);
        if (mainFn is null)
        {
            return null;
        }

        var shape = ClassifyMainSignature(mainFn)!.Value;
        ...
        // Prepare the call to Module.main: build an args List when the user
        // main takes one, then construct the call expression.
        if (shape.HasArgs)
        {
            sb.AppendLine("        var __argList = new global::Overt.Runtime.List<string>(");
            sb.AppendLine("            global::System.Collections.Immutable.ImmutableArray.Create(args));");
        }
        var callExpr = shape.HasArgs
            ? $"global::{ns}.Module.main(__argList)"
            : $"global::{ns}.Module.main()";

        // Two return-shape branches: Int-returning main is the program's exit
        // code directly; Result<Unit, IoError>-returning main maps Ok → 0,
        // Err → 1 (with the error narrative on stderr). The Ok/Err split
        // matches `overt run`'s exit-code contract so the two entry points
        // behave identically when the program is invoked the same way.
        if (shape.ReturnsResult)
        {
            sb.AppendLine($"        var __r = {callExpr};");
            sb.AppendLine("        if (__r.IsOk) return 0;");
            sb.AppendLine("        global::System.Console.Error.WriteLine(__r.UnwrapErr().ToString());");
    ...
    private readonly record struct MainShape(bool HasArgs, bool ReturnsResult);

    /// <summary>Classify <paramref name="fn"/>'s shape against the supported
    /// <c>main</c> entry forms. Returns null for unsupported shapes
    /// (refinement-typed args, multi-arg, returns other than <c>Int</c> or
    /// <c>Result&lt;(), IoError&gt;</c>) so the caller skips entry-point
    /// emission. The two return forms cover the v1 surface: the canonical
    /// Result-returning shape that <c>overt run</c> already supports, and
    /// the simpler Int-returning shape for users who want to emit an exit
    /// code by hand.</summary>
    private static MainShape? ClassifyMainSignature(FunctionDecl fn)
    {
        var hasArgs = fn.Parameters.Length switch
        {
            0 => false,
            1 when fn.Parameters[0].Type is NamedType { Name: "List", TypeArguments.Length: 1 } listT
                && listT.TypeArguments[0] is NamedType { Name: "String", TypeArguments.Length: 0 } => true,
            _ => (bool?)null,
        };
        if (hasArgs is null) return null;

        if (fn.ReturnType is NamedType { Name: "Int", TypeArguments.Length: 0 })
        {
            return new MainShape(hasArgs.Value, ReturnsResult: false);
        }
        if (fn.ReturnType is NamedType { Name: "Result", TypeArguments.Length: 2 } resultT
            && resultT.TypeArguments[0] is UnitType
            && resultT.TypeArguments[1] is NamedType { Name: "IoError", TypeArguments.Length: 0 })
        {
            return new MainShape(hasArgs.Value, ReturnsResult: true);
        }
        return null;
    }
