...
`extern "csharp" fn` binds a host-language member. The binds target's shape
tells the runtime what kind of call to emit:
...
**Opaque types and instance methods.** For stateful BCL types
(`StringBuilder`, `HttpClient`, etc.), declare an `extern type` first and
use `::` to bind instance methods, `..ctor` to bind constructors:
...
The first parameter of an instance-method extern MUST be named `self` and
typed as the target type. The emitter drops `self` from the emitted
argument list and uses it as the C# call receiver: `self.Append(s)`.
Constructors bind to `..ctor` and emit `new T(args)`.
