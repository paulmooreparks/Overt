...
```overt
use stdlib.system.io.path as path

fn main() !{io} -> Result<(), IoError> {
    let combined: String = path.combine_2(path1 = "dir", path2 = "file.txt")
    println(combined)?
    Ok(())
}
```
...
Currently shipped:
- `stdlib.system.io.path` — pure string manipulation (`combine_N`,
  `change_extension`, `get_file_name`, etc.)
- `stdlib.system.io.file` — I/O over file paths (`read_all_text`,
  `write_all_text`, `exists`, etc.)
- `stdlib.system.math` — pure math (`abs`, `min`, `max`, `sqrt`, etc.)
